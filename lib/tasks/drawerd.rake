...
class DrawerdDiagram < RailsERD::Diagram
  ...
  each_relationship do |relationship|
    source, des = relationship.source, relationship.destination
    source_model, des_model = source.model, des.model
    source_table_name, des_table_name = source_model.table_name, des_model.table_name
    ...
    relation_type = case
    when relationship.one_to_one?   then "one"
    when relationship.indirect? && relationship.one_to_many? then "m2m"
    when relationship.one_to_many?  then "many"
    when relationship.many_to_many? then "m2m"
    end
